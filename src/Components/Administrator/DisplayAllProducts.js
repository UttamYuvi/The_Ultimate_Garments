  ...
  function displayCategories() {
    return (
      <MaterialTable
        ...
        {
            title: 'Price', render: (rowData) => <div>
                <div>{rowData.offerprice > 0 ? <><div style={{ fontSize: '12px', fontWeight: 500, color: 'red' }}><s>Price:{rowData.price}</s></div><div style={{ fontSize: '15px', fontWeight: 500, color: 'blue' }}>Offer:{rowData.offerprice}</div></> : <><div style={{ fontSize: '15px', fontWeight: 500, color: 'blue' }}>Price:{rowData.price}</div></>}</div>
                <div style={{ fontSize: '12px', fontWeight: 500 }}>Stock:{rowData.stock}</div>
            </div>
        },

        {
            title: 'Description/Rating', render: (rowData) => <div>
                <div style={{ fontSize: '13px', fontWeight: 500, }}>{rowData.description}</div>
...

        {
            title: 'Status', render: (rowData) => <div>
                <div>{rowData.status === "Continue" ? <><div style={{ fontSize: '15px', fontWeight: 500, color: "green" }}>{rowData.status}</div></> : <><div style={{ fontSize: '15px', fontWeight: 500, color: "red" }}>{rowData.status}</div></>}</div>
                <div style={{ fontSize: '12px', fontWeight: 500, }}>{rowData.salestatus}</div>
            </div>
        },

        { title: 'icon', render: (rowData) => <img src={`${ServerURL}/images/${rowData.picture}`} alt="logo" style={{ width: '40px', borderRadius: 7 }} /> }
